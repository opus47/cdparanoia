 ...
 * the length in seconds of each track e.g, 
 * { 
 *  "tracks": [ 
 *    {"length": 99}, 
 *    {"length": 147}, 
...
static void json_toc(cdrom_drive *d) {
  ...
  json_object_object_add(root, "tracks", arr);
  ...
  for(int i=1; i<=d->tracks; i++){
    if(cdda_track_audiop(d,i)>0) {
      ...
      struct json_object *trk = json_object_new_object();
      json_object_object_add(trk, "length", json_object_new_int(mins*60+secs));

      //json_object_array_add(arr, json_object_new_int(mins*60 + secs));
      json_object_array_add(arr, trk);
...
static void usage(FILE *f){
  fprintf( f,
      ...
      "  -u --unix <socket-file>         : Output progress to unix domain dgram socket.\n"
...
static void init_usock() {
  ...
  psock = socket(AF_UNIX, SOCK_DGRAM, 0);
  name.sun_family = AF_UNIX;
  strcpy(name.sun_path, unix_sock);
...
*/
  ...
  progress=
    ((float)c_sector-(float)callbegin) /
    ((float)callend-(float)callbegin);

  if (unix_sock != NULL) {
    //unix domain socket reporting
    int err = sendto(
        psock, &progress, sizeof(progress), 
        0, 
        (struct sockaddr*)(&name), sizeof(name));

    if (err < 0) {
      perror("error opening report socket");
    }
  ...
  if(!quiet){
    ...
    if(printit==1 || 1){  /* else don't bother; it's probably being 
                        redirected */
      ...
      if(lasttime!=test || function==-1 || slast!=slevel){
        ...
        if(abort_on_skip && skipped_flag && function !=-1){
          ...
          /* progress reporting lives here */
          else
          {
            //cli reporting
            sprintf(buffer,
                "\r (== PROGRESS == [%s| %06ld %02f ] ==%s %c ==)   ",
                dispcache,v_sector,progress,smilie,heartbeat);
...
const char *optstring = "escCn:o:O:d:g:k:S:prRwafvqVQJu:hZz::YXWBi:Tt:l::L::A";
...
int main(int argc,char *argv[]){
  ...
  /*
   * setup socket
   * TODO have option for this
   *
   */

  //bind(psock, (struct sockaddr*)&name, sizeof(name));
  ...
  while((c=getopt_long(argc,argv,optstring,options,&long_option_index))!=EOF){
    switch(c){
      ...
      case 'O':
        ...
        break;
      case 'u':
        printf("copy unix sock %s\n", optarg);
        unix_sock=copystring(optarg);
        init_usock();
        break;
      default:
        usage(stderr);
